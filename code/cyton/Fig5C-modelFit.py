...
Scenario 2 of application of the Cyton2 model fitting CpG-stimulated B cell FACS data.
[Output] Data files (in excel format) for Fig5C, FigS6 and FigS6 -> Run "Fig5-Plot.py" for generating the plots
"""
import sys, os, time, datetime, itertools
import tqdm
import numpy as np
import pandas as pd
...
def fit_tps(inputs):
	...
	for bsample in boots.drop('algo', axis=1).iterrows():
		...
		b_model = Cyton2Model(hts, b_N0, mgen, DT, _nreps, lognorm)
	...
	if lognorm:
		excel_path = f"./out/_lognormal/indiv/{key}_{'_'.join(map(str, comb))}_result.xlsx"
	else:
		excel_path = f"./out/_normal/indiv/{key}_{'_'.join(map(str, comb))}_result.xlsx"
	...
	if lognorm:
		with PdfPages(f"./out/_lognormal/indiv/{key}_{'_'.join(map(str, comb))}tps.pdf") as pdf:
			pdf.savefig(fig1)
			pdf.savefig(fig2)
			pdf.savefig(fig3)
			pdf.savefig(fig4)
		...
		with PdfPages(f"./out/_normal/indiv/{key}_{'_'.join(map(str, comb))}tps.pdf") as pdf:
			pdf.savefig(fig1)
			pdf.savefig(fig2)
			pdf.savefig(fig3)
			pdf.savefig(fig4)
