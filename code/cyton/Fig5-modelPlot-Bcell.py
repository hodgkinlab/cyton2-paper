...
def embed_plot(axis, combination, display, color):
	...
	fit_pars = pd.read_excel(f"{path}/SH1.119_{'_'.join(map(str, combination))}_result.xlsx", sheet_name='pars', index_col=0)
	fit_boots = pd.read_excel(f"{path}/SH1.119_{'_'.join(map(str, combination))}_result.xlsx", sheet_name='boot', index_col=0)
	icnd = 0
	...
	for igen in range(mgen+1):
		# axis.errorbar(rmv_hts, np.transpose(rmv_cgen_avg)[igen], yerr=np.transpose(rmv_cgen_sem)[igen], color=cp[igen], fmt='x')
...
if __name__ == "__main__":
	...
	## Let's see how "ordering" of positions changes with 3 & 9 replicates
	path1 = './out/_all_Lognormal/indiv/dt=0.5_B cells -TPS_9Rep_tk2'
	path2 = './out/_all_Lognormal/indiv/dt=0.5_B cells -TPS_3Rep_tk2'
	df_fits1, df_fits2 = [], []
	for i, rm in enumerate(np.arange(1, 9, step=1)):
		tp_idx = np.arange(0, 10, step=1)
		removes = np.array(list(itertools.combinations(tp_idx, rm)))
		for j, comb in enumerate(removes):
			try:
				fit1 = pd.read_excel(f"{path1}/SH1.119_{'_'.join(map(str, comb))}_result.xlsx", sheet_name='boot', index_col=0)
				fit1.drop(['mUns', 'sUns', 'mDiv0', 'sDiv0', 'mDD', 'sDD', 'mDie', 'sDie', 'm', 'p', 'N0', 'algo'], axis=1, inplace=True)
				fit1['tps'] = fit1.apply(lambda x: '-'.join(map(str, comb+1)), axis=1)
				fit1.index = [r"$k={0}$".format(len(comb))] * len(fit1.index)
				df_fits1.append(fit1)

				fit2 = pd.read_excel(f"{path2}/SH1.119_{'_'.join(map(str, comb))}_result.xlsx", sheet_name='boot', index_col=0)
				fit2.drop(['mUns', 'sUns', 'mDiv0', 'sDiv0', 'mDD', 'sDD', 'mDie', 'sDie', 'm', 'p', 'N0', 'algo'], axis=1, inplace=True)
				fit2['tps'] = fit2.apply(lambda x: '-'.join(map(str, comb+1)), axis=1)
				fit2.index = [r"$k={0}$".format(len(comb))] * len(fit2.index)
				df_fits2.append(fit2)
			except:
				pass
	df_fits1 = pd.concat(df_fits1)
	df_fits2 = pd.concat(df_fits2)

	fig, ax = plt.subplots(nrows=2, ncols=3, figsize=(8,6))
	ax = ax.flatten()
	# red_cp = sns.color_palette('gist_rainbow', n_colors=len(np.unique(df_fits1.index)))
	red_cp = sns.color_palette(n_colors=len(np.unique(df_fits1.index)))
	for i, lab in enumerate(np.unique(df_fits1.index)):
		df_agg1 = df_fits1[df_fits1.index==lab].groupby('tps').agg(np.median)
		df_agg1.sort_values('rmse-out', ascending=True, inplace=True)
		order1 = df_agg1.index.to_list()

		df_agg2 = df_fits2[df_fits2.index==lab].groupby('tps').agg(np.median)
		df_agg2.sort_values('rmse-out', ascending=True, inplace=True)
		order2 = df_agg2.index.to_list()

		mapped = []
		for j, item in enumerate(order2):
			position = order1.index(item)
			mapped.append([j, position])
		mapped = np.array(mapped)
		ax[i].set_title(f"{lab} ({len(mapped)})", x=0.01, ha='left', color=red_cp[i])
		ax[i].scatter(mapped[:,0],mapped[:,1], s=5, color=red_cp[i])
		lims = [
			np.min([ax[i].get_xlim(), ax[i].get_ylim()]),  # min of both axes
			np.max([ax[i].get_xlim(), ax[i].get_ylim()]),  # max of both axes
		]
		ax[i].plot(lims, lims, 'k--', zorder=0, lw=1)
		if i == 0:
			...
			ax[i].xaxis.set_major_formatter(ticker.FormatStrFormatter('%d'))
			ax[i].xaxis.set_minor_locator(ticker.MultipleLocator(2))

			ax[i].yaxis.set_major_locator(ticker.MultipleLocator(10))
			ax[i].yaxis.set_major_formatter(ticker.FormatStrFormatter('%d'))
			ax[i].yaxis.set_minor_locator(ticker.MultipleLocator(2))
		elif i in [2, 3, 4]:
	...
	fig.text(0.5, 0.04, "Sorted position (3 replicates)", ha='center', va='center')
	fig.text(0.02, 0.5, "Corresponding sorted position (9 replicates)", ha='center', va='center', rotation=90)
	fig.tight_layout(rect=(0, 0.03, 1, 1))
	fig.subplots_adjust(wspace=0.25, hspace=0.32)
	fig.savefig(f"{path2}/f0_3rep_vs_9rep_order.pdf", dpi=300)
	...
	for path in reversed(paths):
		...
		for irg, (a, b) in enumerate(ranges):
			...
			for i, rm in enumerate(rms[a:b]):
				...
				for j, comb in enumerate(removes):
					try:
						fit = pd.read_excel(f"{path}/SH1.119_{'_'.join(map(str, comb))}_result.xlsx", sheet_name='boot', index_col=0)
						...
						pass
				print(f" >> -{rm} combinations: {count} counts")
			...
			# ref_fits = pd.read_excel(f'{path}/SH1.119_result.xlsx', sheet_name='boot_9reps', index_col=0)
			ref_fits = pd.read_excel(f'{path}/SH1.119_result.xlsx', sheet_name='boot_3reps', index_col=0)
			...
			# ax1[0].annotate(f"[Reference] All time points + 9 replicates", xy=(pos-0.23, 0.97), xycoords=('data', 'axes fraction'), va='top', weight='bold', fontsize=12, rotation=90)
			...
			if irg == 0:  # show minor tick values
				...
				# inset cyton plots
				# exs = [np.array([1, 6]), np.array([3, 4]), np.array([1, 5, 7]), np.array([2, 3, 4])]  # for 9 replicates
				exs = [np.array([1, 7]), np.array([3, 4]), np.array([1, 5, 7]), np.array([2, 3, 4])]  # for 3 replicates
				idx = []
				for ex in exs:
				...
				# exs = [np.array([1, 5, 7, 8]), np.array([1, 3, 5, 7]), np.array([3, 4, 5, 8]), np.array([2, 3, 4, 7])]
				exs = [np.array([1, 2, 3, 7]), np.array([1, 3, 5, 7]), np.array([3, 4, 5, 8]), np.array([2, 3, 4, 7])]  # for 3 replicates take 2
				idx = []
				for ex in exs:
					string = '-'.join(map(str, ex+1))
					for index, label in enumerate(ax1[1].xaxis.get_ticklabels()):
				...
				exs = [np.array([1, 2, 4, 5, 7]), np.array([1, 2, 4, 5, 6, 7])]  # for 9 replicates
				idx = []
				for ex in exs:
					string = '-'.join(map(str, ex+1))
					for index, label in enumerate(ax1[1].xaxis.get_ticklabels()):
	...
	for path in reversed(paths):
		...
		ax1.set_xlabel(f"PC1 ({out['explained_var'][0]*100:.1f}% explained variance)")
		ax1.set_ylabel(f"PC2 ({(out['explained_var'][1]-out['explained_var'][0])*100:.1f}% explained variance)")
		...
		for irep, rep in enumerate(reps):
			...
			inputs.append((boot_df, irep, rep, df, irep+1, 1000000))  # decide number of bootstrap samples
		...
		ax3[0,0].set_ylabel("CV")
		ax3[0,0].set_xlabel("# Replicate")
		...
		ax3[1,0].set_ylabel(f"$\Delta$CV")
		ax3[1,0].set_xlabel(f"$\Delta$#Replicate")
		...
		ax3[1,1].set_xlabel(f"$\Delta$#Replicate")
