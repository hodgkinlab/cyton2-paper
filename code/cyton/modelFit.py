...
def bootstrap(key, df, cond, icnd, hts, nreps, params, paramExcl, lognorm):
	...
	for b in tqdm_trange1:
		...
		#  - I'm only intereseted in t > TIME_THRESHOLD, but the activation fraction depends on the empirical average initial cell numbers. Thus, I must shuffle them all to reflect the bootstrap changes on fA estimation
		cells = df['cgens']['rep'][icnd]  # n(g,t): number of cells per gen at t
		x_boot, y_boot, N0 = [], [], 0 # generations & raveled n(g,t)
		for data, ht in zip(cells, hts):
			tmp = 0
			for _ in range(len(data)):
				rand_idx = rng.randint(0, len(data))
				for igen, cell_number_rep in enumerate(data[rand_idx]):
					x_boot.append(igen)
					y_boot.append(cell_number_rep)
					tmp += cell_number_rep
			if not N0:
				N0 = tmp / len(data)
		# convert from python lists to numpy arrays
		x_boot = np.asfarray(x_boot)  # generation numbers
		y_boot = np.asfarray(y_boot)  # number of cells per generation

		model = Cyton15Model(hts, N0, int(max(x_boot)), DT, nreps, lognorm)  # define cyton model object
		...
		for s in tqdm_trange2:
			...
			try:  # Some set of initial values is completely non-sensical, resulted in NaN errors
				mini_lm = lmf.Minimizer(residual, pars, fcn_args=(x_boot, y_boot, model), **LM_FIT_KWS)
				...
				# mini_de = lmf.Minimizer(residual, params, fcn_args=(x_boot, y_boot, model), **DE_FIT_KWS)
				...
				tqdm_trange2.set_description(f" > {key} > {''.join(cond.split())}")
			...
			# except:
			# 	err_count += 1
			# 	print(f"[BOOTSTRAP][{err_count}] Error on {key} > {cond} > {sys.exc_info()[0]}")
...
# def fit(key, df, reader, icnd, lognorm=True):
def fit(inputs):
	key, df, reader, icnd, lognorm = inputs
	...
	for s in tqdm_trange:
		...
		try:  # Some set of initial values is completely non-sensical, resulted in NaN errors
			...
			# mini_de = lmf.Minimizer(residual, params, fcn_args=(x_gens, y_cells, model), **DE_FIT_KWS)
			# res_de = mini_de.minimize(method='differential_evolution', max_nfev=MAX_NFEV)  # Differential evolution
			...
			tqdm_trange.set_description(f"[SEARCH] > {key} > {''.join(condition.split())}")
	...
	### RUN BOOTSTRAP
	boots = bootstrap(key, df, condition, icnd, hts, nreps, params, paramExcl, lognorm)
	...
	writer.save()
	writer.close()
	# print(f"\n-----> [{key}][{condition}] FIT REPORT")
	# print(lmf.fit_report(fit_results.iloc[0]['result']))
	# print(f"-----> [{key}][{condition}] SUMMARY")
	...
	fig1.suptitle(f"[{key}][{condition}] Cyton parameters, Total cohort and cell numbers")
	...
	fig4.suptitle(f"[{condition}] Bootstrap distribution")
	...
	grid.fig.suptitle(f"[{condition}] Parameter correlation")
	# grid.fig.set_size_inches(14, 10)
	grid.fig.set_size_inches(rc['figure.figsize'][0], rc['figure.figsize'][1])
	grid.fig.tight_layout()

	if lognorm:
		with PdfPages(f"./out/_all_Lognormal/indiv/{key}_{condition}.pdf") as pdf:
			pdf.savefig(fig1)
			pdf.savefig(fig2)
			pdf.savefig(fig3)
			pdf.savefig(fig4)
		...
		with PdfPages(f"./out/_all_Normal/indiv/{key}_{condition}.pdf") as pdf:
			pdf.savefig(fig1)
			pdf.savefig(fig2)
			pdf.savefig(fig3)
			pdf.savefig(fig4)
...
if __name__ == "__main__":
	...
	PATH_TO_DATA = './data'
	DATA = ["DV-19MI_010.xlsx"]#"EX127.xlsx", "EX130b.xlsx", "DV20-02.xlsx", "SH1.119.xlsx"]
	...
	for key in KEYS:
		...
		for icnd in range(len(reader.condition_names)):
			inputs.append((key, df[key], reader, icnd, LOGNORM))
	...
	## ALTERNATIVE MULTIPROCESSING METHOD: WON't ALIGN TQDM PROCESS BAR PROPERLY
	# procs = []
	# for key in KEYS:
	# 	reader = df[key]['reader']
	# 	for icnd in range(len(reader.condition_names)):
	# 		p = mp.Process(target=fit, args=(key, df[key], reader, icnd, LOGNORM))
	# 		p.start()
	# 		procs.append(p)
	# for p in procs:
	# 	p.join()
